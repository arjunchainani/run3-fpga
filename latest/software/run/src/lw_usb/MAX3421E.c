...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	const uint8_t wr_buffer[2] = {reg + 2, val};
	if (XSpi_Transfer(&SpiInstance, wr_buffer, NULL, sizeof(wr_buffer)) != XST_SUCCESS) {
		xil_printf("Write failed! (MAXreg_wr(), MAX3421E.c)");
	}

	XSpi_SetSlaveSelect(&SpiInstance, 0); // deselect
}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	//return (data + nbytes);
	if (data == NULL) return NULL; // avoid segfaults

	XSpi_SetSlaveSelect(&SpiInstance, SpiInstance.SlaveSelectMask);

	uint8_t wr_buffer[nbytes + 1];
	wr_buffer[0] = reg + 2;

	for (int i = 0; i < nbytes; i++) {
		wr_buffer[i + 1] = data[i];
	}

	if (XSpi_Transfer(&SpiInstance, wr_buffer, NULL, sizeof(wr_buffer)) != XST_SUCCESS) {
		xil_printf("Write failed! (MAXbytes_wr(), MAX3421E.c)");
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	const uint8_t wr_buffer[2] = {reg, 0};
	uint8_t rd_buffer[2] = {0};

	if (XSpi_Transfer(&SpiInstance, wr_buffer, rd_buffer, sizeof(wr_buffer)) != XST_SUCCESS) {
		xil_printf("Read failed! (MAXreg_rd(), MAX3421E.c)");
	}

	XSpi_SetSlaveSelect(&SpiInstance, 0); // deselect

	return rd_buffer[1]; // return val
}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	uint8_t wr_buffer[nbytes + 1]; // including dummy bytes to keep the duplex transmission running
	memset(wr_buffer, 0, nbytes + 1);
	wr_buffer[0] = reg;

	uint8_t rd_buffer[nbytes + 1];

	if (XSpi_Transfer(&SpiInstance, wr_buffer, rd_buffer, nbytes + 1) != XST_SUCCESS) {
		xil_printf("Read failed! (MAXbytes_rd(), MAX3421E.c)");
	}

	XSpi_SetSlaveSelect(&SpiInstance, 0); // deselect

	for (int i = 0; i < nbytes; i++) {
		data[i] = rd_buffer[i + 1];
	}
